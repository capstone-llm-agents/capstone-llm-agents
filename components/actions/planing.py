...
async def plan_router(prompt: str, actions: List[Any], Agents: List[Any]):
    ...
    llm_prompt = (f'''You are the "Router" for an AI agent. Your job is to analyze the user's request and determine the most appropriate strategy for handling it. You must choose one of the following three strategies. Respond with ONLY a single JSON object representing your choice.
...
class Plan(Action):
    ...
    async def do(self, params: ActionParams, context: ActionContext) -> ActionResult:
        ...
        APP_LOGGER.info(f"{result}")
        if result['strategy'] == 'direct_answer':
            APP_LOGGER.info(f"direct_answer")
            context.plan = f"Use {SimpleResponse.__name__}"
            res = ActionResult()
            res.set_param("response", "")
            return res
        elif result['strategy'] == 'single_action':
            APP_LOGGER.info(f"single_action")
            context.plan = f'Perform the following action {result}'
            res = ActionResult()
            res.set_param("response", "")
            return res
            ...
            prompt = [{"role": "user", "content": f"{messages[-1]}"}] + [
                {
                    ...
                    "content": f"""
                                        ...
                                        The plan must be a JSON object containing a list of steps. Each step must be an object with a "tool_name" and "arguments".

                                        You have access to the following resources:
                                        - Actions: {action_name}
                                        - Other Agents: {friends}

                                        Carefully analyze the user's request and the available tools.

                                        If the request is impossible to fulfill with the available tools, your plan should consist of a single step using the tool `simpleresponse`.

                                        Your final output must be only the JSON plan object."""
                }
            ]
