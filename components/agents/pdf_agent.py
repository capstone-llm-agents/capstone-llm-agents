"""The pdf agent module demonstrates how to create a simple agent with actions and workflows."""
...
PDF_AGENT = Agent(
    "PDFAgent",
    "An assistant that can create and manage PDF documents.",
    action_space,
    narrower,
    selector,
    tool_manager,
...
# add some actions
PDF_AGENT.add_action(RespondWithChatHistory())
PDF_AGENT.add_action(StopAction())
PDF_AGENT.add_action(UpdateTools(tool_creator))
PDF_AGENT.add_action(GetTools(tool_creator))
PDF_AGENT.add_action(GetRelevantTools(tool_creator, embedding_model=ModelsAPI.get_embedding))
PDF_AGENT.add_action(GetParamsForToolCall(tool_creator))
