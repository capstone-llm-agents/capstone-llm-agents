...
GITHUB_AGENT = Agent(
    "GitHubAgent",
    "An assistant that helps manage and provide GitHub repository information.",
    action_space,
    narrower,
    selector,
    tool_manager,
...
# add some actions
GITHUB_AGENT.add_action(RespondWithChatHistory())
GITHUB_AGENT.add_action(StopAction())
GITHUB_AGENT.add_action(UpdateTools(tool_creator))
GITHUB_AGENT.add_action(GetTools(tool_creator))
GITHUB_AGENT.add_action(GetRelevantTools(tool_creator, embedding_model=ModelsAPI.get_embedding))
GITHUB_AGENT.add_action(GetParamsForToolCall(tool_creator))
