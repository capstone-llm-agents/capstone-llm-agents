...
from llm_mas.utils.config.app_config import AppConfigManager
...
class GeneralConfig:
    ...
    def __init__(self, models_path: str, vector_path: str, app_path: str) -> None:
...
config = None
try:
    config = GeneralConfig("config/models.yaml", "config/vector.yaml", "config/app.yaml")
except Exception as e:
    msg = f"Failed to initialize configuration manager. {e} \n\n Please update or create the valid configuration files. It is recommended to run `python init_config.py` to create the config an initial config. However, this config will have to be updated to use the models you prefer."  # noqa: E501
