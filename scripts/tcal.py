...
from icalendar import Calendar as Calendar2#different to ics Calendar. if we get to cleaning up the code i could look into finding a way to just use one. for now to make things quicker i will keep both
...
if task == 1:
    ...
    result = format_user_request("Maths at 10am for two hours, english at 6pm for 1 hour and history at 12pm tommorow", datetime.now().date())
    print(result["content"])
    print("\n")
    ...
    result = agent_read_and_write_calender("Between maths and english book me a 1 hour meeting with Anton.", datetime.now().date(), file_name_read, file_name_write)
elif task == 4:
    print("task 4")
