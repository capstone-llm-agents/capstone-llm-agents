...
async def main() -> None:
    ...
    async with sse_client(url=server_url) as streams:  # noqa: SIM117
        async with ClientSession(read_stream=streams[0], write_stream=streams[1]) as session:
            ...
            ics_file = "scripts/my_callender.ics"
            result = await session.call_tool(
                "create_ics_callender",
                {
                    "tasks": "Maths at 10am for two hours, english at 6pm for 1 hour and history at 12pm tommorow",
                    "ics_file": ics_file,
                },
            )
            ...
            ics_file_read = "scripts/export_test.ics"
            ...
            ics_file1 = "scripts/my_callender.ics"
            ics_file2 = "scripts/my_callender_reformatted.ics"
            result = await session.call_tool(
                "create_ics_callender_with_context",
                {
                    "tasks": "Between maths and english book me a 1 hour meeting with Anton.",
                    "file_name_read": ics_file1,
                    "file_name_write": ics_file2,
                },
            )
